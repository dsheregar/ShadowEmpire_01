...
function RenderHurtboxes(){	
	var head = CreateHurtboxes(0, 0, 0, 0, obj_hurtbox_head)
	var body = CreateHurtboxes(0, 0, 0, 0, obj_hurtbox_body)
	var low = CreateHurtboxes(0, 0, 0, 0, obj_hurtbox_low)
	...
	if character == CHAR_RANJIT
	{
		switch (state)
		{
			...
			case STATE_STEPBACK:
				head.image_xscale = 28 * sign(image_xscale)
				head.image_yscale = 23
				head.xOffset = -30 * sign(image_xscale)
				head.yOffset = -160
				
				body.image_xscale = 60 * sign(image_xscale)
				body.image_yscale = 57
	...
	if character == CHAR_MIGUEL
	{
		switch (state)
		{
			case STATE_FREE:
				if landed
				{
					...
					else
					{
						...
						low.image_xscale = 30 * sign(image_xscale)
						low.image_yscale = 50
						low.xOffset = 1 * sign(image_xscale)
						low.yOffset = -70
					}
			...
			case STATE_ATTACK:
				...
				body.image_xscale = 30 * sign(image_xscale)
				body.image_yscale = 50
				body.xOffset = -8 * sign(image_xscale)
				body.yOffset = -135
				...
				switch(dmg_state)
				{
					case EFF_KNOCKUP:
						...
						} else
						{
							head.image_xscale = 20 * -sign(image_xscale)
							head.image_yscale = 20
							head.xOffset = -12 * -sign(image_xscale)
							head.yOffset = -140
